...
Scans every `.vue` and `.js` file of the islands for `t('…')` and appends the missing keys
to `lang/{app.locale}.json` — key as value, so the interface stays readable until someone
translates the line. Existing lines keep their order; unused keys are listed, never removed.

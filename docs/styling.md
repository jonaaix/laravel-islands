...
A few helpers take defaults from a provider so that a project can decide once:

```js
// resources/js/app.js
import { BUTTON_DEFAULTS_KEY, CARD_DEFAULTS_KEY } from '@aaix/laravel-islands/vue/helpers';

startVueIslands(registry, {
    setup(app) {
        app.provide(BUTTON_DEFAULTS_KEY, { shape: 'pill', size: 'md' });
        app.provide(CARD_DEFAULTS_KEY, { mediaRatio: '3 / 2' });
    },
});
```

The same can be done for a subtree with `provideButtonDefaults()` and
`provideCardDefaults()` from inside a component. An explicit prop always wins.

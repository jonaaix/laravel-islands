...
`select.skins|menu`, `popover.surface`, `badge.base|tones`, `choice.boxes` (checkbox and radio),
`fileField.zones`, `skeleton.surface`, `tooltip.surface|arrow`, `modal.sizes|surface|divider`, `toast.surface|tones`,
...
track, floating panels (select menus, popovers, modals, toasts, tooltips) lift on a tinted
shadow with a faint edge instead of a hard ring, badges are pills, an unchecked box, a file
zone and a field group are filled rather than outlined, corners are generous (`0.75rem`
controls, `1.25rem` cards) and the neutral is Tailwind's cooler slate. It replaces `field.base` — the box every input draws — which is the
